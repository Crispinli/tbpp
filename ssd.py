...
class SSD(nn.Module):
    ...
    def __init__(self, phase, size, base, extras, head, num_classes):
        ...
        self.priorbox = PriorBox(self.cfg) # calculate the size of prior boxes
        self.priors = Variable(self.priorbox.forward(), volatile=True)
        self.size = size

        # SSD network
        self.vgg = nn.ModuleList(base) # nn.ModuleList([xxx, yyy, ...])
    ...
    def load_weights(self, base_file):
        other, ext = os.path.splitext(base_file) # os.path.splitext(path) -> other, ext
        if ext == '.pkl' or '.pth':
